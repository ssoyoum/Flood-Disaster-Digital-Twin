...
@lru_cache(maxsize=8)
def _connected_building_ids(event_id: str) -> frozenset[int]:
    """Return IDs present in the connected building inventory.

    This function only validates that a requested feature exists. It does not
    turn a derived flood envelope into a risk score or intervention outcome.
    """
    layers = get_layers(event_id)
    building_features = layers.get("buildings", {}).get("data", {}).get("features", [])
    return frozenset(
        _building_id(feature, index)
...
def run_scenario(scenario: ScenarioRecord) -> ScenarioRunResult:
    ...
    connected_ids = _connected_building_ids(scenario.event_id)
    unknown_ids = sorted(set(requested_ids) - connected_ids)
    if unknown_ids:
        raise ValueError(f"Unknown building_ids for {scenario.event_id}: {unknown_ids}")
    assumptions = [
        "The requested building IDs were validated against the connected building inventory.",
        "After-intervention flood extent, depth, traffic/vehicle exposure, and building vulnerability data are not connected.",
        "No intervention-effect coefficient or priority threshold is applied without a source-backed model.",
        "Risk reduction and before/after priority counts are therefore unavailable, not zero.",
    ]
    return ScenarioRunResult(
        ...
        status="UNAVAILABLE",
        before_priority_buildings=None,
        after_priority_buildings=None,
        risk_reduction=None,
        before_risk_score=None,
        after_risk_score=None,
        priority_building_ids_before=[],
        priority_building_ids_after=[],
        assumptions=assumptions,
    )


def validate_scenario_buildings(event_id: str, building_ids: list[int]) -> None:
    connected_ids = _connected_building_ids(event_id)
    unknown_ids = sorted(set(building_ids) - connected_ids)
    if unknown_ids:
...
def analyze_closure_timing(event_id: str, closure_times: list[str]) -> dict:
    ...
    return {
        ...
        "limitations": [
            ...
            "Underpass water depth and a validated detection threshold are not connected.",
        ],
    }
